...
class ChattingLog{
    ...
    create(roomID, question, answer){

        return new Promise((resolve, reject)=> {
            models.chatting_log.create({
                room_id : roomID,
                question : question,
                answer : answer
            }).then(response => {
        ...
        return new Promise((resolve, reject) => {
            models.chatting_log.findAll({
                ...
                if(response.length > 0){
                    ...
                    return reject(message.issueMessage(message["404_NOT_FOUND"], "CHATTING_LOG_NOT_FOUND"))
                }
            }).catch(error => {
                return reject(message.issueMessage(message["500_SERVER_INTERNAL_ERROR"], "UNDEFINED_ERROR"))
            })
