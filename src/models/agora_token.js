const AccessToken = require("./rtctoken").AccessToken2;
const ServiceRtc = require("./rtctoken").ServiceRtc;
const ServiceRtm = require("./rtctoken").ServiceRtm;
...
class RtcTokenBuilder {
  ...
  static buildTokenWithUid(
    ...
    const issueTs = Math.floor(new Date().getTime() / 1000); // 현재 시간으로 설정
    const token = new AccessToken(appId, appCertificate, 0, tokenExpire); // issueTs 추가
    // const uidStr = String(uid); // uid를 문자열로 변환
    // const serviceRtc = new ServiceRtc(channelName, uidStr); // 문자열로 변환된 uid 사용
  ...
  static buildTokenWithUserAccount(
    ...
    const issueTs = Math.floor(new Date().getTime() / 1000); // 현재 시간으로 설정
    const token = new AccessToken(appId, appCertificate, issueTs, tokenExpire);
